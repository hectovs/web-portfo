    ...
    update_data:(value: null | object[] | ((prevState: null | object[]) => null | object[])) => void;}) 
{
    ...
    return (
        <div>
                                            ...
                                            columns.map((column: object, column_index: number )=>{
                                                return(
                                                    <td className="border" key={"data" + data_index + "column" + column_index}>
                                                        <input  className="shadow appearance-none border rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline "   type="text" readOnly value={datum[column["jsonKey"]]}/>
                                                    </td>
                                                )
                                            })
                                        }
            ...
            <button className="bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 rounded" onClick={()=>{update_data([...data, inputRow])}}>
                Add row 
            </button> 
        </div>
    )
