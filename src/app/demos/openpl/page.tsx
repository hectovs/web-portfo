...
export default function openPl() {
    ...
    const [equipmentOptions, setEquipmentOptions] = useState<string[]>(["Raw", "Single-ply", "Multi-ply", "Wraps"])
    ...
    useEffect(() => {
        ...
        fetch("/api/lifterscsv/" + lifter)
            ...
            .then((data) => {
                ...
                console.log(oplJson)
                setData(oplJson.data.sort((a,b)=>Date.parse(a["Date"]) - Date.parse(b["Date"])))
            })
        setLoading(false)
    }, [lifter])
    ...
    const transformToScatter = (data: object[], equipment: string[], chartDataDisplay: object[])=>{
    ...
    useMemo(() => {
        transformToScatter(data, equipment, chartDataDisplay)

    }, [data, equipment, chartDataDisplay])
    ...
    return (
        <div className="w-full">
            <div className="w-full py-2 px-4 flex flex-row h-16">
                <label className="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" htmlFor="lifter">
                    Lifter Name
                </label>
                <input className="shadow appearance-none border rounded py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:shadow-outline mx-2" id="lifter" type="text" value={lifterInput} onChange={(event) => { setLifterInput(event.target.value) }} />                
                <button className="bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 rounded mx-2" onClick={() => { setLifter(lifterInput) }}>
                    Update Lifter Data
                </button>
                <label htmlFor="equipment">Equipment Type</label>
                <select 
                    id="equipment"
                    className="shadow appearance-none border rounded py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:shadow-outline mx-2 w-48"
                    multiple={true}
                    value={equipment}
                    onChange={(event)=>{
                ...
                >
                    {equipmentOptions.map((eO)=>{ 
                        return(<option key={eO} value={eO}>{eO}</option>)
                    })}
                </select>
            </div>
            ...
            <div className="w-full h-1/4 overflow-y-auto">
